...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session || !isAdmin(session.role)) {
      console.error("Admin avatar POST: No session or not admin")
    ...
    console.log("Saving avatar for admin user:", session.sub)
    ...
    console.log("Verified admin user avatar_url length:", checkUser?.avatar_url?.length)
